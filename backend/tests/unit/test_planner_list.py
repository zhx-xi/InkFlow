...
def _session(**kw: object) -> PlannerSession:
    ...
    return PlannerSession(
        ...
        created_at=kw.pop("created_at", _now()),  # type: ignore[misc]  # 测试 helper：kw.pop 键值覆盖返回 object
        updated_at=kw.pop("updated_at", _now()),  # type: ignore[misc]  # 测试 helper：kw.pop 键值覆盖返回 object
    )
