...
from inkflow.domain.services.agent_service_context import AgentServiceContextMixin
...
class AgentService(AgentServiceStreamMixin, AgentServiceContextMixin):
    ...
    async def _merge_role_configs(
        ...
        for stage in stages:
            ...
            if project_model == AGENT_DEFAULT_SENTINEL:
                new_agent.model = (
                    resolve_model(None, project_config.model, config.llm_default_model) or ""
                )
            elif project_model:
    ...
    async def _run_pipeline(
        ...
        # #1349：注入明细落库（与 stream_pipeline 同一机制）
        injection_detail = await self._inject_context(
            context, continue_context=continue_context, override=override
        )
        await self._persist_injection_detail(execution_id, injection_detail)
        try:
