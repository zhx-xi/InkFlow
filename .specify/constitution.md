...
- 测试策略（至少 3 个关键测试场景）
- 不在范围内（Out of Scope）
- 非功能需求（数据规模、可接受延迟、降级行为 —— 各 1 行即可，无则显式写「不适用」）
- 验证方式（§13 每项验收标准的可执行命令；**优先 `pytest -k <关键词>` 选择器**，若写文件路径则基准为 `backend/tests/`）

> §13 验证方式避免写死深路径：`pytest -k <关键词>` 对目录重构免疫；写路径时基准一律 `backend/tests/`（#1190 实测 151 条路径因 #636 重构失效）。
