...
| 命令 | 必选参数 | 可选/易错 | 说明 |
|---|---|---|---|
| `audit chapter <章节>` | 位置参数 `chapter`（名称或 UUID） | `--project/-p`（名称或 ID，**非 --project-id**）`--include-static/--no-include-static` `--confirm(accept\|reject)` `--note` `--history` | 触发章节审计并输出结果；`--history` 可省（跳过历史） |
...
- `--project` 参数名是 `--project` 不是 `--project-id`（接受名称或 ID）
