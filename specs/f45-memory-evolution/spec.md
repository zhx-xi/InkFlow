...
**依赖**：M2 依赖 M1（先有跨项目聚合，LLM 才有跨项目证据可总结）。M2 建议接编排器阶段 4（长跑）之后——长跑产生大量修改证据喂给 difflib 锚点；但**不硬阻塞**：可先用 F28 既有证据跑通（设计定稿 §5）。
...
- **注入前自动检查**：PreferenceSource.collect 时若锚点哈希 ≠ 既有总结哈希 → 触发惰性总结（后台不阻塞注入：先用旧总结注入 + 审计 pending_summary，见 §7 边界表；或同步总结——Q2 待澄清）。
- 锚点变化后的总结时机为 M2 关键权衡（成本 vs 新鲜度），见 §14 Q2。
...
PreferenceSource.collect(project_id, chapter_id):
  ...
  5. 冲突过滤沿用 F28 Q4 规则（value 命中显式设定文本 → 跳过）
...
| 项目被删除 | 服务层级联清理：该项目 project_preferences/memory_events（F28 既有）+ 用户级偏好**保留但重算**（source_projects 移除该项目；若 project_count < 2 → 该用户级偏好删除，降级回项目级证据不足） | 无（F27 先例） |
| 全部项目被删除 | user_preferences 全清、semantic_summaries 清空（scope=user） | 无 |
...
| 锚点变化但总结未刷新（惰性待总结） | collect 先用旧总结注入 + 审计 pending_summary；或同步总结（Q2 拍板） | 无（降级不阻断） |
...
| CREATE | `backend/tests/unit/test_memory_service_user.py` | M1 编排契约（零行为/落库/删除/项目删除重算） |
...
| 编排服务（M1） | ⑥ memory_learning=false 零行为（无用户级写入/注入/审计）⑦ user_preferences CRUD ⑧ 删除后 collect 不含该条 ⑨ 项目删除重算（source_projects 移除/project_count<2 删除）⑩ stats 输出 user 层计数 | ≥90% |
...
| 注入源（M2） | ⑱ 语义总结优先 + 字面兜底 ⑲ 项目级/用户级归属 title 区分 ⑳ 预算 10 条/200 字符 ㉑ 冲突过滤延续 | ≥90% |
...
| 编排器长跑证据喂料（M2 ← 编排器阶段 4 咬合） | F44（0.10.0）——M2 可先用 F28 既有证据跑通，不硬阻塞（设计定稿 §5） |
...
- **被依赖**：F44 编排器（阶段 4 长跑产生修改证据喂 difflib 锚点，M2 软咬合）、F19 GUI（渲染层接入偏好面板/toast，远期）。
...
- **M1-2 编排服务全绿**: `pytest tests/unit/test_memory_service_user.py` — memory_learning=false 零行为 + user_preferences CRUD + 项目删除重算 + stats user 层计数
...
- **Q1: 项目删除时用户级偏好的重算时机** 🔲 待拍板
  ...
  - 建议：A（同步重算——删除语义确定性优先，F28「删除立即生效」精神延续；估算 0.5 人天）
- **Q2: 锚点变化后语义总结的触发时机（M2 成本 vs 新鲜度）** 🔲 待拍板
  ...
  - 建议：A（同步总结——无后台任务基建依赖、锚点幂等控成本；估算 0.5 人天 vs B +1 人天）
- **Q3: M1 用户级偏好注入是否也受「显式设定冲突过滤」约束** 🔲 待拍板
  ...
  - 建议：A（一致性 + 防跨项目覆盖显式设定；实现 = PreferenceSource.collect 已加载 explicit_texts，用户级条目复用同一过滤，≈0 额外人天）

---

**完成门禁对照**（本 spec 交付时）：13 节 + §14 待澄清 Q1-Q3（≤3）；围栏偶数；marker 残留 0；参照 F28 spec（525 行）体量目标 ≤800 行；M1 覆盖 #339、M2 覆盖 #340（两段式架构 + 两层归属 + user_preferences 表）、LLM 配置遵循 #415。
