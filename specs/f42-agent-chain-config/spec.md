...
**Spec 版本**: 1.1（拍板修订：Q1 层级自由排序 / Q2 真禁用 / Q3 格式统一，2026-08-12）
**日期**: 2026-08-12
**依据**: 0.8.0 路线图拍板记录 5-9（design/inkflow-0-8-0-roadmap-2026-08-12.md）+ Issue #268（Agent 链模型选择）+ Issue #269（Agent 执行顺序编辑）+ #225 三态语义（已合入）+ F26/F27 已合入实现源码核查 + 2026-08-12 用户拍板（待澄清 Q1-Q3 全部确认）
...
- **不含** 自定义 Agent 的执行节点扩展：`agent_order` 数据模型/校验**支持任意角色名**（为自定义 Agent 铺路），但执行节点映射（`_NODE_MAP`）仍限 4 内置角色——自定义角色出现在 agent_order 时存储/校验允许、执行层剔除 + warning（§5.3.4；自定义 Agent 执行 = 0.9.0 F39-F41 多 Agent 域）。
...
class ProjectConfig(BaseModel):
    ...
    """Agent 链执行拓扑 — 层级嵌套数组（Q1 拍板：1、2、3、4 序号，同层并行）。

    - 外层 = 执行层序（第 1 层先执行，逐层串行）
    - 内层 = 同层并行角色字段名数组（agent_architect 等，同层角色并行执行）
    - 示例: [["agent_architect"], ["agent_writer", "agent_auditor"], ["agent_reviser"]]
            = 层 1 架构规划 → 层 2 写作+审阅并行 → 层 3 修订
    - 空列表 = 未配置 → 默认拓扑 [[architect],[writer],[auditor],[reviser]]（零迁移）
    - 角色名支持任意字符串（自定义 Agent 铺路）；执行节点映射限 4 内置（§5.3.4）
    """

    @field_validator("agent_order")
    @classmethod
    def validate_agent_order(cls, v: list[list[str]]) -> list[list[str]]:
        """存储层校验：结构 + 去重（语义校验在 API 层与执行层，§2.3）。

        - 每层为非空字符串列表；空层（[]）拒绝
        - 元素跨层全局去重（同角色出现两次拒绝，防歧义）
        - 单角色便捷写法 [["agent_writer"]] 与 [["agent_writer"]] 等价（内层恒为数组）
        """
        seen: set[str] = set()
        result: list[list[str]] = []
        for layer in v:
            if not isinstance(layer, list) or not layer:
                raise ValueError("agent_order 每层必须为非空数组")
...
| 层 | 行为 | 位置 |
|----|------|------|
| 存储层（Pydantic） | 结构 + 跨层去重（§2.1） | `domain/models/project.py` |
| API 层（PATCH） | 语义校验：非法 → **422**（detail 中文说明）；非法定义 = ① 含未知角色字段名（非 4 内置且非「启用角色」范围内）② 跨层重复（存储层已拒，防御）③ **不含全部启用角色**（Q2 拍板口径：启用 = agent_* 非 null；关闭角色可不出现在 agent_order） | `api/routers/project.py`（或 `project_service.py`，实现确认） |
| 执行层（防御） | 任何非法（含存量数据手工损坏）→ **回退默认拓扑**，记 warning 日志；未知角色名 → 从执行链剔除 + warning（§5.3.4）；永不抛错中断管线 | `agent_service.py` |
...
| 场景 | 状态码 | detail |
|------|--------|--------|
| agent_order 含未知角色字段名 | 422 | 「agent_order 包含未知角色: xxx」（执行层剔除 + warning，§5.3.4） |
| agent_order 缺启用角色（Q2 口径） | 422 | 「agent_order 必须包含全部启用角色: xxx」 |
| agent_order 空层/重复角色 | 422 | 「agent_order 每层必须为非空数组」/「agent_order 角色重复: xxx」 |
...
3. 语义校验（执行层防御）：非法（未知角色/缺启用角色/空层）→ 记 warning + 原样返回（回退默认）
...
#### 5.3.2 引擎放宽（`langgraph_pipeline.py` MODIFY）
...
#### 5.3.4 自定义角色边界（Q1 拍板铺路，不在本模块实现执行）

- `agent_order` **数据模型与校验支持任意角色字段名**（§2.1/§2.3）——用户可编排含自定义 Agent 的列表，UI 不限制。
- 执行层：`_NODE_MAP` 仍限 4 内置角色（architect/writer/auditor/reviser）；未知角色名出现在 agent_order → **执行层剔除 + warning 日志**（不影响其余角色执行）——诚实降级而非谎称支持。
- 自定义 Agent 的执行节点（pipeline_nodes 扩展 + 模板集成）＝ **0.9.0 F39-F41 多 Agent 域**（#257 能力白名单登记已就绪）；本模块保证数据层不堵死（层级模型天然容纳任意角色）。
...
| agent_order 含未知角色名 | API 422；执行层剔除 + warning（§5.3.4） | 输入拒绝 / 防御剔除 |
| agent_order 缺启用角色（Q2 口径） | API 422；执行层回退默认 | 输入拒绝 / 防御回退 |
| agent_order 空层/跨层重复 | 存储层/API 422 | 输入拒绝 |
...
| 自定义 AgentTemplate 项目 | agent_order 仅作用于内置四角色（§5.3.4 边界：模板拓扑不重排，模板角色覆盖仍生效） | 无（文档化行为） |
...
| MODIFY | `backend/src/inkflow/infrastructure/agent/langgraph_pipeline.py` | validate 放宽多入口/多终点（L57-62）；execute 多 START/END 边（L120-126） |
| MODIFY | `backend/src/inkflow/infrastructure/agent/pipeline_nodes.py` | `_build_messages` 变量注入 `.get` 防御 + 空注入（L58-64） |
...
| 层次 | 关键场景 | 覆盖率目标 |
|------|---------|-----------|
| 模型/契约（后端） | ProjectConfig.agent_order 默认空/结构/空层拒绝/跨层重复拒绝；agent_* 三态 validator 回归 | ≥90% |
...
| 集成（后端） | LangGraphAgentPipeline 层级拓扑 validate() 通过（多入口/多终点）；并行层 mock LLM 并发调用（层序确定、层内并行）；环检测回归；未执行角色空注入（`state["results"]` 缺 key 不炸） | ≥90% |
...
| 自定义 Agent 执行节点（pipeline_nodes 扩展 + 模板集成任意角色） | 0.9.0 F39-F41 多 Agent 域（#257 能力白名单登记已就绪；本模块仅数据层铺路 §5.3.4） |
...
- **M4 层级拓扑 + 校验（#269 后端）**: `pytest backend/tests/unit/test_project_config_order.py` + `test_langgraph_pipeline.py` — agent_order 层级结构/空层/重复拒绝；PATCH 语义校验 422（缺启用角色/未知角色）；存量项目零迁移；**并行层 mock LLM 并发调用（层序确定、层内并行、变量空注入防御）**；环检测回归
- **M5 执行拓扑重排（#269 集成）**: `pytest backend/tests/unit/test_agent_service.py` — `_apply_agent_order` 层级重排（含 `[["architect"],["writer","auditor"],["reviser"]]` 并行场景）/跳过过滤/空回退/非法回退；全连接边断言（input_from=前序全部、output_to=后序全部）；重排后 validate() 恒通过
- **M6 层级编辑 UI + 持久化（#269 验收闭环）**: GUI 每角色序号 1-N（同序号=并行组）→ PATCH agent_order 层级结构 → 重启保持；写作流程按层序执行（内核 stderr / 执行日志可验证：层间串行、层内并行）；保存时对「未来层引用/同层互引」给出空输入提示
