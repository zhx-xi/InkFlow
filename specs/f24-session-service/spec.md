...
>
> **Spec 变更**（1.0 → 1.2）: ① 1.1 删除语义按用户拍板修订为**两级删除**（首次 DELETE = 归档可解除；已归档再 DELETE = 真实删除；force 直删）——§2.5/§3.1/§3.2/§4/§7/§9/§12/§13 同步；② 1.2 待澄清 Q1/Q2/Q3 全部拍板（2026-08-07）：Q1 project_id 可空维持 / Q2 终态允许追加日志维持 / Q3 会话↔执行 MVP 软关联（新增 §5.4b 可观测键约定 + §12 决策 9 + §10 归位）
>
> **所属阶段**: 0.5.0 Agent 集成（第 1 个模块，估算 2-3 人天）
>
> **关联 Issues**: #51（F24 会话管理）；#52（F25 daemon，**被依赖**）
>
> **依赖**: ✅ F1（项目实体，project_id 可空引用）· ⏳ F25（daemon 后台写作，本模块是被依赖方）· ⏳ F3（写作会话上下文快照，Phase 2+ 联动）
>
...
| 跨模块 MODIFY | 无（纯新增；F25 消费本模块端口，本模块不反向依赖） |
...
| 粒度 | 一次管线运行（write_chapter 全流程） | 一次业务会话（写作会话可含多次执行；任务会话对应一次 daemon 任务） |
| 消费方 | F4 agent_service / 调试 | CLI/API/未来 GUI/F25 daemon（履历查询） |
...
- **会话承载任务履历**：daemon 定时写作（F25）基于会话——任务触发来源多样（用户 CLI 设定 / chat 指令 / 其他 agent 经 MCP/skill 调用），统一以会话记录生命周期 + 履历日志，用户可查看任务履历/历史日志
- **F25 依赖 F24**：F25 spec 基于本 spec 定稿（硬依赖）
- **F25 daemon 形态**：综合（CLI 子命令 + 后台常驻 + 进度通知）——进度通知写入本模块 SessionLog（本 spec §5.4 预留进度写入契约）
...
| context | dict[str, Any] | NOT NULL, DEFAULT {} | **上下文快照**（JSON）：写作会话 = 恢复续写所需的上下文（项目/章节/模式/参数快照）；任务会话 = 任务参数（daemon 目标/调度/LLM 配置）。恢复语义见 §5.3 |
...
- `project_id` 可空：会话不强制绑定项目——任务履历可能跨项目（如全局定时任务）；写作会话由调用方决定是否挂项目（F25 定时写作挂目标项目）
...
| 备选方案 | 优点 | 缺点 | 结论 |
|----------|------|------|------|
| **类型字段 + 统一上下文快照（选定）** | 单表零复杂度；context/result 为 JSON 自由结构，写作/任务各自约定键；类型只决定默认行为与校验 | 类型特定字段无强类型约束（依赖 JSON 键约定） | ✅ MVP——会话是**轻量载体**，深层语义由消费方（F3/F25）约定；强类型子类表 YAGNI（§10） |
...
- `paused` → `active`（resume）：恢复暂停的会话；**恢复语义** = 会话重新可被消费（F25 续跑任务 / 写作恢复上下文），见 §5.3；清空 paused_at
...
> 创建会话时若携带 `project_id`，校验项目存在（404，**前置校验先于创建**——同 F13 `_ensure_project` 模式）；`project_id=null` 不校验。终态会话（completed/failed）追加日志**允许**（履历补记，如 daemon 写最终结果）——不设终态只读（见待澄清 Q2）。
...
```
 ...
 (CLI/F25/API)──▶│  ② 状态机: pause/resume/complete/fail        │
                 │  ③ 履历: 追加日志（进度/错误/结果）            │
...
- **幂等性**：MVP **不承诺幂等**（重复 complete → 第二次 422 非法迁移）——与 F11「生成即新建」一致：动作语义是「迁移」而非「确保状态」，重复调用明确报错（调用方应查状态）。幂等重试归 F25 daemon（§10）
...
- **任务会话（task）**：`context` 快照携带任务参数（调度/目标/范围）；F25 daemon `resume` 后读取 context 续跑未完成任务。**这是 F25 硬依赖 F24 的核心契约**：daemon 的「暂停/恢复/进度」全部落在会话状态机 + 日志上
- 恢复的**进度连续性**：以日志 seq 为序查看已执行步骤（`GET /logs` 即任务履历），消费方据 `last_log`/`result` 决定续跑起点（F25 的详细续跑算法在其 spec 定义，本模块只保证「日志可完整回溯」）

### 5.4 进度写入契约（F25 消费预留）

F25 daemon 进度通知按以下约定写入会话（本 spec 定义**契约形状**，F25 实现遵守）：
...
**规则**：
- 键**全部可选**——F25 daemon 0.5.0 只填 `execution_id`/`agent_name`/`stage`/`duration_ms`（§5.4 表已含的键优先），`tool_calls` 留空（F26 工具化后才可能填充）
...
| 13 | 日志 `seq` 冲突（并发追加） | DB_ERROR 500（MVP 不处理并发日志追加——daemon 单任务串行写日志，见 §10；幂等重试归 F25） |
| 14 | 大 context/result JSON | 允许（SQLite JSON 列，无独立大小限制；调用方控制体积，F25 约定 ≤ 64KB） |
...
| daemon 调度/定时器/进程生命周期 | 任务执行与调度是 F25 daemon 职责（本模块只提供会话载体） | F25（0.5.0） |
| 幂等重试 / 断点续跑算法 | 会话保证「日志可回溯」，续跑决策由消费方（F25）实现 | F25（0.5.0） |
...
| 通知/推送（桌面通知、系统托盘） | daemon 的进度通知 = 会话日志（§5.4）；系统级推送归 F25 或 GUI | F25/未来 |
...
F24 被依赖:
  F25（daemon）           — 任务会话载体 + 履历日志 + 暂停/恢复状态机（硬依赖，用户拍板 2026-08-07）
  F26+（Agent 化）        — agent 任务履历可复用会话（未来）
```

**编号口径声明**：PRD v2.1 §6.2 中 F8-F17 一行式引用 v2.0 原文，「会话管理」在 v2.0 编号为 F16（旧）；现行编号以 ADR-019 v4 为准：F24 = 会话管理、F25 = daemon（旧文档中指向会话管理的「F16」编号已过时）。Issue #51 body 中 `specs/ff24-<name>/spec.md` 为双 f 笔误（F14 先例同款），实际目录 `specs/f24-session-service/`。
...
| 2 | 状态机四态 + 动作端点 | active/paused/completed/failed，pause/resume/complete/fail 专用端点 | 任务履历需要暂停/恢复语义（F25 硬依赖）；迁移规则单一入口可校验（F13 模式） | PATCH 直改 status（任意跳转无法校验，F13 否决同款）；两态 open/closed（无暂停语义，不满足 PRD「暂停恢复」） |
...
| 8 | 动作不承诺幂等 | 重复 complete → 422 | 动作语义 = 迁移（非确保状态），重复调用明确报错；幂等重试归 F25 | complete 幂等（第二次返回已 completed——掩盖调用方状态盲区） |
...
| # | 问题 | 影响 | 结论 |
|---|------|------|------|
| Q1 | 会话是否**必须**挂项目？ | 数据模型约束与 API 校验（§2.1/§3.2） | ✅ 已确认（用户拍板 2026-08-07：按设计）——`project_id` **可空**；三类场景支撑（程序内 chat AI 辅助任务、跨项目/全局任务如 F25 定时写作、项目删除后履历保留），详见正文 §2.1/§6.1 |
...
| Q3 | 会话与 F4 AgentExecution 是否 MVP 建立关联？ | 数据模型字段与 F25 消费方式（§2.1/§5.4b） | ✅ 已确认（用户拍板 2026-08-07：**MVP 软关联，不加 FK**）——SessionLog.payload 可观测键（execution_id/agent_name/stage/tool_calls/duration_ms，§5.4b）承载「查看当前 agent 运行/执行到哪一步/工具调用细节」；正式 FK 与 CI/CD 式执行视图归 0.7.0 Agent 化升级（F27/F28），届时零迁移 |
