...
管理小说的**结构化大纲**（卷/章/节规划层的创建/查询/更新/真删）、**情节点**（故事关键节点，有序、带类型）与**故事弧线**（跨章节的情节发展线，情节点的组织维度），并支持**基于项目设定用 LLM 自动生成大纲**（大纲名/情节点/弧线）。
...
**业务规则**:
- `name` 项目内**大纲唯一** = 「同名 = 同一规划版本」，防止手误重复建档；删除后再创建同名大纲合法（旧行已物理删除，全唯一索引）
- **大纲删除 = 物理删除（v1.1 / #211）**：大纲真删 → 其情节点级联物理删除（服务层级联 + DB FK CASCADE）；无软删/恢复语义；**弧线不受影响**（弧线是项目级组织维度，见 §2.3）
...
|------|------|------|------|
| id | UUID | PK | 领域层 UUID |
| outline_id | UUID | NOT NULL, FK→outlines.id (CASCADE), 已索引 | 所属大纲（大纲真删 → 级联物理删除） |
...
| arc_id | UUID? | NULLABLE, FK→story_arcs.id (SET NULL), 已索引 | 所属故事弧线（可选；弧线真删 → 置 NULL） |
...
- 情节点**删除 = 物理删除**（v1.1）；弧线真删 → 成员情节点 arc_id 置 NULL（情节点本身保留，同 F9 分组删除语义）
...
| id | UUID | PK | 领域层 UUID |
| project_id | UUID | NOT NULL, FK→projects.id (CASCADE), 已索引 | 所属项目 |
| name | str | NOT NULL, 1-50 字符, 去空白 | 弧线名（如「主角成长线」「反派线」）；**项目内弧线唯一**（全唯一索引，见 §2.4） |
...
- 弧线**真删** → 成员情节点 arc_id 置 NULL（不级联删情节点）；无恢复语义（v1.1，同 F9 分组删除语义）
...
**为什么改用全唯一索引（v1.1 / #211）**: 「项目内大纲/弧线同名唯一」防止手误重复建档，也是 AI 生成时**弧线按名复用**（§5.4）的锚点；`is_deleted` 列移除后不存在「软删行」，partial 过滤失去对象；删除后再创建同名实体仍合法（旧行已物理删除，不占唯一性）。服务层仍做一次同名检查以给出友好 422 文案。
...
| DELETE | `/api/v1/outlines/{outline_id}` | 删除大纲（**v1.1 真删 + 情节点级联删除**） | — | 204 |
| ~~POST~~ | ~~`/api/v1/outlines/{outline_id}/restore`~~ | **（v1.1 移除）** 端点已不存在 | — | — |
...
| DELETE | `/api/v1/plot-points/{point_id}` | 删除情节点（**v1.1 真删**） | — | 204 |
| ~~POST~~ | ~~`/api/v1/plot-points/{point_id}/restore`~~ | **（v1.1 移除）** 端点已不存在 | — | — |
...
| DELETE | `/api/v1/story-arcs/{arc_id}` | 删除弧线（成员 arc_id 置 NULL；**v1.1 真删**） | — | 204 |
| ~~POST~~ | ~~`/api/v1/story-arcs/{arc_id}/restore`~~ | **（v1.1 移除）** 端点已不存在 | — | — |
...
{
  "id": "9b1c2d3e-...", "project_id": "3f2e1d4a-...", "name": "第一卷大纲",
  "description": "...", "sort_order": 1, "extra": {},
...
**删除大纲（v1.1 真删语义）**（级联语义见 §7）:
```http
DELETE /api/v1/outlines/9b1c2d3e-...   → 204（物理删除，情节点级联物理删除）
```
> **v1.1（#211）变更**：原 `DELETE` 默认软删 + `?force=true` 物理删双路径**收敛为真删单路径**；原 `POST /outlines/{id}/restore`（级联恢复情节点）端点**移除**，删除后不可恢复。
...
| 存在同名（v1.1 真删语义下无「已软删」状态，旧行已物理删除） | 视为不存在 → 创建新大纲 | 落库 |
...
- **级联规则（v1.1 真删）**：大纲真删 → 情节点级联物理删除（服务层：`hard_delete_outline` 内先删情节点再删大纲 / DB FK CASCADE）
- 已删除的大纲/情节点物理不存在，不进入任何查询结果
...
| 创建大纲名与项目内大纲重复 | 422: "同名大纲已存在（大纲名在项目内必须唯一）" |
| 删除后**再创建同名大纲** | ✅ 成功（全唯一索引 v1.1；旧行已物理删除，不占唯一性） |
| 大纲 description > 5000 / sort_order < 0 | 422（字段校验） |
| 获取/更新/删除不存在的大纲 | 404: "大纲不存在" |
| 大纲删除（v1.1 真删） | 204；其情节点**级联物理删除**，弧线不受影响 |
| 重复删除同一大纲 | 404: "大纲不存在"（不可恢复） |
...
| 创建弧线名与项目内弧线重复 | 422: "同名弧线已存在（弧线名在项目内必须唯一）" |
| 获取/更新/删除不存在的弧线 | 404: "弧线不存在" |
| 弧线删除（v1.1 真删） | 204；成员情节点 arc_id 置 NULL（情节点保留） |
| 生成时项目不存在 | 404: "项目不存在" |
...
| 项目硬删除 | 大纲/情节点/弧线级联物理删除（FK CASCADE） |
...
└── cli/
    ├── commands/
    │   ├── outline.py             ← CREATE: outline 组（create/list/get/update/delete/
...
class OutlineRepositoryProtocol(Protocol):
    ...
    按 spec §2.4: 项目内大纲/弧线 name 唯一（全唯一索引 v1.1 / #211）；
    删除即物理删除，不存在「已删行」，故无 partial 过滤。大纲真删 → 情节点
    级联（服务层编排 + FK CASCADE，无 soft_delete_points_of / restore_points_of）；
    弧线真删 → 成员 arc_id 置 NULL（clear_arc_of_points）。
    ...
    async def hard_delete(self, outline_id: int) -> bool: ...   # v1.1 真删（唯一删除路径）
    # v1.1 移除: soft_delete / restore / soft_delete_points_of / restore_points_of（#211）
    ...
    async def hard_delete_point(self, point_id: int) -> bool: ...   # v1.1 真删
    # v1.1 移除: soft_delete_point / restore_point（#211）
    ...
    async def hard_delete_arc(self, arc_id: int) -> bool: ...   # v1.1 真删
    # v1.1 移除: soft_delete_arc / restore_arc（#211）
```
...
**仓储**: 三实体 CRUD 往返 / `get_by_name`（大纲/弧线）命中与未命中 / 同名唯一（全唯一索引 v1.1：插入第二个同名 → IntegrityError；删除后可再插同名）/ 已删除行物理不存在（get 返回 None）/ `next_position`（空大纲 → 1、追加 → max+1）/ `list_points` position ASC 稳定排序 / `list_points_by_arc` 按弧线聚合 / `clear_arc_of_points`（弧线删除 → 成员 arc_id NULL）/ 大纲真删级联情节点 / `list` 搜索与分页 / 弧线列表 name ASC / FK 级联（大纲真删 → 情节点级联；项目真删 → 三实体级联）

**服务**: 三实体创建/更新/真删全流程 / 同名大纲 → 422 / 同名弧线 → 422 / arc_id 跨项目或不存在 → 422 / 大纲不存在各操作 → None → 404 / 大纲真删 → 级联情节点物理删除编排 / 弧线真删 → 成员 arc_id 置 NULL 编排 / generate 入口编排（项目不存在 → 404；generator/project_repo 未注入 → 配置错误）
...
| 唯一约束实现 | 全唯一索引（v1.1 / #211；原 partial unique `WHERE is_deleted = 0`） | `is_deleted` 列移除后无「已删行」需排除；删除后再创建同名实体仍合法（旧行已物理删除，不占唯一性）；比「仅服务层检查」更稳（DB 兜底，同 F9/F10） |
...
| 级联规则 | 大纲真删 → 情节点级联物理删除；弧线真删 → 成员 arc_id 置 NULL（不级联删情节点） | 大纲是情节点的容器（容器级联）；弧线是组织维度（同 F9 分组语义：解除关联不删成员） |
...
|------|---------|--------------|------|------|------|
| POST /projects/{project_id}/outlines | 项目存在 | 校验 name（非空/≤50/项目内唯一）→ 创建 | 201 + Outline | 404「项目不存在」；422「大纲名不能为空」/「大纲名不能超过 50 个字符」/「同名大纲已存在（大纲名在项目内必须唯一）」 | description>5000 / sort_order<0 → 422；删除后同名可再建（全唯一索引 v1.1） |
...
| DELETE /outlines/{outline_id} | 大纲存在 | **真删 + 情节点级联物理删除** | 204 | 404「大纲不存在」 | **v1.1**：无软删路径、无 `force` 参数；重复删除 → 404；弧线不受影响 |
| ~~POST /outlines/{outline_id}/restore~~ | — | **（v1.1 移除）** 端点已不存在 | — | 请求该路径 → 404 | — |
...
| DELETE /plot-points/{point_id} | 情节点存在 | **真删** | 204 | 404「情节点不存在」 | **v1.1** 无软删 |
| ~~POST /plot-points/{point_id}/restore~~ | — | **（v1.1 移除）** 端点已不存在 | — | — | — |
...
| DELETE /story-arcs/{arc_id} | 弧线存在 | **真删**（成员情节点 arc_id 置 NULL，情节点保留） | 204 | 404「弧线不存在」 | **v1.1** 无软删 |
| ~~POST /story-arcs/{arc_id}/restore~~ | — | **（v1.1 移除）** 端点已不存在 | — | — | — |
...
- A2：同名大纲 → 422「同名大纲已存在（大纲名在项目内必须唯一）」；删除后再建同名 → 成功
- A3：DELETE outlines → 204 后其情节点级联物理删除（GET plot-points 404）；不可恢复
- A4：generate save=false → 200 + preview 且不创建任何实体；save=true 同名 → 422
- A5：generate LLM 输出无法解析（重试后仍失败）→ 500「大纲生成失败: LLM 输出无法解析，请重试」
- A6：弧线真删 → 204 且成员情节点 arc_id 置 NULL（情节点保留）

### 14.4 Spec 漂移标注（追加时核对实现 routers/outlines.py）

- **restore 端点（v1.1 已移除，非实现缺口）**：§3.1/§4.1 原声明的 `POST /outlines/{outline_id}/restore`、`POST /plot-points/{point_id}/restore`、`POST /story-arcs/{arc_id}/restore` 及 CLI `outline restore` 在 routers/outlines.py / services / CLI 中**均不存在**——这是 #211「普通实体软删→真删」的**有意结果**（非未落地）：v1.1 删除即物理删除，无恢复场景，故端点/命令一并移除。本节曾据实现缺失记为漂移，现更正定性（2026-09-18 文档同步补齐）。§3.1 表头计数已随移除端点更新。
