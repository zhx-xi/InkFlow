...
> **🔴 实现规范（否则事件会缺 project_id）**：B 类方法**必须在发布前先取得实体**——实测这些方法**本就已加载实体**（如 `update_character` 内部 `existing = await self._repo.get(...)`；`delete_character` 先取实体以触发 map_cleanup 钩子），因此 **`project_id = str(existing.project_id)` 可零额外查询获得**。**禁止**为发事件新增一次 `repo.get()` 查询（违反「事件是尽力而为信号」的轻量定位）。
>
> **已知例外（B 类中 project_id 拿不到的情形）**：`delete_chapter` 实测实现为**直接透传 repo**（`return await self._repo.delete_chapter(_to_int(chapter_id))`，无实体加载）——此方法需**新增一次实体查询**（或改由 repo 返回被删实体的 project_id）才能带上 project_id。**同类风险**：`delete_pin`、`delete_relation`、`delete_category` 等「薄透传」方法。**实现期逐方法核对**，拿不到 project_id 时**发 `None` 并记 warning**（不阻断写入，不伪造 id）——GUI 侧该事件退化为「全项目刷新」（§15.6.3）。
...
> **🔴 安全偏向原则（本章硬约束）**：**「防数据丢失」类校验（#989）的双保险不可拆**；**「可见性」类补丁（#973）可在推送验证后移除**。区别判据 = **该陈旧会导致数据损坏/丢失吗？** 会 → 保留兜底；只会「看不到」→ 可移除。
...
| # | 问题 | 选项 | 建议 |
|---|------|------|------|
| Q15-1 | **失效粒度**：事件到达后是「全量重拉」还是「局部更新」？ | A. **一律 FR 全量重拉**（本章 D15-1，复用既有 loadXxx，实现最简）；B. 逐域 LU 局部更新（省流量但需逐域推算，错漏风险高）；C. 混合（列表 FR + 高频面 LU） | **建议 A**——本地自用数据量小，FR 成本可忽略；LU 的「推算错反而更隐蔽陈旧」风险大于收益（#989 即 FR 治好的） |
| Q15-2 | **#989 类「防数据丢失」补丁是否保留**？ | A. **长期保留为最后防线**（本章 D15-4）；B. 推送验证后删除（靠推送唯一保障） | **建议 A**——推送尽力而为，删补丁 = 把数据丢失风险的唯一防线押在可能断的流上（本章 §15.7.2 硬约束） |
| Q15-3 | **首批覆盖域（批次 A）范围**？ | A. **本章 §15.3.4 的 8 域**（map/map_pin/agent_template/settings/provider_config/agent/character/outline）；B. 仅 3 个爆点域（map/agent_template/settings）；C. 一次性全 17 域 | **建议 A**——覆盖三爆点 + 同构高危面，既有代表性又不铺太开；C 违反「不做架构宇航员」 |
| Q15-4 | **批次 B（其余 15 域）是否本迭代做**？ | A. **挂 0.14.0 但视余量顺延**（批 0+A1+A2 为本迭代目标）；B. 本迭代全做完；C. 全部顺延到下迭代 | **建议 A**——先让机制在批次 A 上验证（含 E2E 与补丁收编），再铺量 |

> **Q15 与 issue #992 验收的对应**：Q15-1/Q15-2 影响 §15.6/§15.7（验收项「失效粒度表」/「收编」）；Q15-3/Q15-4 影响 §15.13 排期。**拍板后本节标 ✅ 已确认并修订正文对应位置。**
