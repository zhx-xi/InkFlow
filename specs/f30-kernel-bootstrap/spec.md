...
> **Spec 版本**: 1.0 | **日期**: 2026-08-07 | **依据**: ADR-030（本地内核服务化 ②）、ADR-021（内核进程化交付契约）、Constitution P1-P6
>
...
| # | 问题 | 影响 | 建议 |
|---|------|------|------|
| Q1 | 冷启动**默认等待超时**取多少？spec 设计为 30s（内核冷启动含 chromadb/BGE 加载，实测 ~4.7s 但打包后可能更慢）；是否需要可配置（env `INKFLOW_KERNEL_TIMEOUT`）？ | 影响 agent 调用体验（超时 = 明确报错 vs 无限等待） | 建议 A：**默认 30s + env 可覆盖**——agent 超时预算通常 60s，30s 留出余量；可配置应对慢机器。若倾向简单选 B（固定 30s 不配置） |
| Q2 | **版本校验严格度**？spec 设计为 major 相同即复用（minor/patch 容忍）；是否要 `>=` 客户端版本（内核必须不旧于客户端）？ | 影响契约漂移风险 vs 复用率 | 建议 A：**major 相同即复用**——API 向后兼容（ADR-019 冻结语义），minor/patch 差异通常兼容；`>=` 会在旧内核 + 新客户端时强制拉起（多一次冷启动）。若担心 API 不兼容选 B（内核版本 >= 客户端才复用） |
| Q3 | `kernel status` 调试命令是否保留？spec 设计为保留（dev 标注）；或 MVP 不提供（纯库，靠测试验证）？ | 影响调试面 vs 最小面 | 建议 A：**保留**——集成测试/排障直接受益（`inkflow kernel status` 比读 JSON 文件直观），成本极低（~30 行）。若倾向极简选 B（去掉，靠 kernel.json 手工查看） |

---

*本文档为 F30 功能规格（What），实施步骤（How）见后续 `specs/f30-kernel-bootstrap/plan.md`。所有里程碑验收以本节 M1-M7 为准。*
