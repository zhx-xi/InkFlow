...
- 原型引用：design/GUI/memory/memory.html + memory-<state>.png（add-form/extracting/stats）
...
│项目　　　　　　│　项目　青云志▾ 　　　　　　　　　　　　　　　　　│
│设定库　　　　　│　┌─────────────────────┐　│
│角色　　　　　　│　│统计概览：记忆总量　42 ・ 项目偏好　37　　│　│
│世界观　　　　　│　│用户偏好　3 ・ 语义总结　2　　　　　　　　│　│
│大纲　　　　　　│　│Agent 行为：章节　21 ・ 直接确认　46　　　│　│
│时间线　　　　　│　│修改率 38% ・ 重写率 12% ・ 平均改动 214　│　│
│伏笔　　　　　　│　└─────────────────────┘　│
│知识图谱　　　　│　［提取记忆］　　［添加记忆］　　　　　　　　　　│
│检索　　　　　　│　┌─────────────────────┐　│
│会话　　　　　　│　│项目偏好（来源：项目级）　　　　　　　　　│　│
│Agent 　　　　　│　│［称呼］　6 ・ 置信度　0.92 　　　　　　　│　│
│记忆　●　　　　│　│苏云舟 → 主角，称「云舟」；师父称「掌门」│　│
│设置　　　　　　│　│［编辑］　［删除］　　　　　　　　　　　　│　│
│［折叠］　　　　│　└─────────────────────┘　│
│　　　　　　　　│　┌─────────────────────┐　│
│　　　　　　　　│　│用户级偏好（来源：用户级）　　　　　　　　│　│
...
| 日期 | 变更 | 说明 |
|------|------|------|
| 2026-09-11 | #1098 缺陷修复（后端数据源扩展，本页零改动） | 「提取记忆」此前仅在「用户编辑过写作草稿」的项目可成功——记忆锚点/事件源唯一来自 draft 域，导致只有 agentic/planner/执行会话的项目恒返回 warn toast「暂无可提取的记忆内容」。后端扩展会话事件源后（`session_completed` 事件 + stats 口径 + 锚点纳入会话文本，见 `specs/f28-memory-learning/spec.md` §5.7.1 / `specs/f45-memory-evolution/spec.md` §5.7.1），本页**交互与文案零变更**：`summarized=true` → ok toast「记忆提取完成」；`false` → warn toast「暂无可提取的记忆内容」（空项目仍给可读提示）。§2 动作表与 §3 验收 N3 语义不变。 |
