...
| 新建对话框 · 书名 | 文本输入框 | 输入书名 | — | — | 提交时 strip 空 → 内联「书名不能为空」；>100 字 → 「书名不能超过 100 字」 | 必填 1-100；错误内联展示，对话框保持打开可改 |
| 新建对话框 · 标签 | 预设标签多选 Select + 自定义输入 + 已选 chips | 点选切换选中态；自定义 Enter 新增（去重非空）；chip × 移除 | — | 标签集合实时更新 | — | 未选任何标签 → 提交时报「请至少选择一个题材/标签」；建议来自本地轻量注册表（tags 聚合 + 旧 genre 枚举） |
