...
> **Spec 变更**：2026-09-22 #1374（原型阶段；⏳ 实现待用户确认后落地）—— **双序轴向语义分流**：叙事序轴 = **章**（章刻度 + 章下事件）；世界序轴 = **世界内时间**（单轴、无章分组、行尾 = 来源章）。世界序「纪元分轴」留 0.16.0（#1353，事件无 era 字段）。筛选入规格（按章 / 按类型；按世界 / 按角色 = 0.16.0）。原型：design/GUI/timeline/timeline.html（状态 narrative / narrative-filter / world / world-multiaxis / empty）。
...
> ⚠️ **#1374 目标形态（原型已产出；⏳ 实现待用户确认后落地）** —— 下方锚点描述的是 #1323 交付的**当前实现**，实现轨落地后按新形态更新。

### §1.1 双序轴向定义（#1374，⏳ 待实现）

| 序 | 轴刻度 | 排序键 | 分组 | 行内主信息 | 行尾副信息 |
|----|--------|--------|------|------------|------------|
| **叙事序** | **章**（章节标题原样；「第 N 章」序号由 #999 标题归一化保证，前端零拼接） | 章序（章节列表顺序）→ 章内 `narrative_position`（#1323 合成序） | 按章分组（容器 `tl-chgroup-<chapterId>`，刻度 `tl-chtick-<chapterId>`） | 事件标题；世界内时间为行内小字（ink-3 降级，`tl-axis-main-<id>`） | —（章信息即刻度） |
...
- **筛选维度**（#1374 新增入规格）：按章（✅ `source_chapter_id` + 章节列表）、按事件类型（✅ `timeline_flag`：正叙/倒叙/插叙）；按世界/纪元（❌ → 0.16.0 #1353）、按角色关联（❌ 无正式字段，`extra` 预留）——后两者 0.15.0 不排期
- **图例**（`lib.tlLegend` 按序拆两 key，⏳ 待实现）：叙事序「轴=章 · 事件按章推进排列；行内小字=世界内时间」/ 世界序「轴=世界内时间（升序，未知末尾）· 不再按章分组；行尾=来源章」
...
|------|--------|--------|--------|------|------|------|
| 双序 chips（tl-view-narrative / tl-view-world） | 叙事序默认激活 | 切换**轴语义**（#1374 ⏳）：叙事序 → **章轴**（章刻度 + 章下事件）；世界序 → **时间轴**（全局时间序、无章分组）。当前实现：本地切换显示数组（章分组不变，仅组内顺序变） | — | 列表即时切换（零额外请求，T2/T3 契约） | — | 叙事序组内 narrative_order 空 → 回退 event_timeline（现行）；轴语义分流后该回退仅影响叙事序组内排序 |
| 筛选（tl-filter-chapter / tl-filter-type，#1374 ⏳） | 描边按钮「章：全部」「类型：全部」 | 展开下拉面板（按章多选 / 按事件类型多选）；勾选即过滤 | — | 列表即时过滤（对两序共用）；按钮高亮 + 标签回显（如「章：第十二章」） | — | 「全部」= 不过滤；按世界 / 按角色 = 0.16.0 不排期（数据面缺字段） |
...
| 章分组 header（当前实现 tl-group-title-<chapterId>；#1374 ⏳ 改为**章刻度** tl-chtick-<chapterId>） | 纯文本展示（真实章节标题） | — | — | — | — | 无交互；映射缺失 → 「未知章节」；未归章 → 「未分章」 |

## 3. 验收

- N1：双序 chips 切换**轴语义**（#1374 ⏳：叙事序 = 章轴 / 世界序 = 时间轴）；叙事序组内 narrative_order 空回退 event_timeline（#1323 现行）
...
- N5：**轴刻度语义**（#1374 ⏳）—— 叙事序刻度 = **章**（章节标题原样，`tl-chtick-<chapterId>`）；世界序刻度 = **世界内时间**；图例随序切换（`lib.tlLegend` 拆两 key）；行内时间小字（叙事序降级 ink-3）
- N6：**叙事序 = 章轴**（#1374 ⏳）—— 按 source_chapter_id 分组为章刻度（容器 `tl-chgroup-<chapterId>`）；刻度用**真实章节标题**，**不含由 narrative_position 拼出的「第N章」**（#1323 G2）；组内按章内叙事序排列
- N7：**每个事件恰好渲染一次** —— tl-axis-node-<id> 数量 == displayed 长度，事件标题在页面上各出现一次（不再「轴 + 列表」两块重复渲染）（#1323 G1）；**世界序无章分组容器**（#1374 ⏳）
- N8：**未分章 / 未知章节落位**（#1374 ⏳）—— 叙事序：未归章事件落轴末尾灰刻度（`tl-chtick-__none__`，文案「未分章」），映射缺失刻度退化「未知章节」；世界序：行尾胶囊显示来源章（未分章 / 未知章节同文案）（#1323 G3 的落位语义保留）
- N9：**筛选**（#1374 ⏳）—— 按章筛选 → 列表仅显示该章事件（两序共用）；按事件类型筛选（正叙/倒叙/插叙）；重置 = 「全部」
