...
| Q2 | F6 `ForeshadowingSource` 空实现（`infrastructure/context/sources.py`）与 `api/deps.py` 装配的替换是否纳入 F13 里程碑？该文件属 F6 模块，但真实实现依赖 F13 Repository；F9/F10 的先例是「只交付实体/查询能力，替换作为 0.2.0 内 F6 联调任务」 | 影响 F13 收尾范围与 F6 文件修改归属（sources.py/deps.py MODIFY）；不纳入则验收标准 2「写作时注入伏笔提示」无法实证 | 建议：**纳入 F13**（本 spec §5.3/§8/§13 已按此设计）——与 F9/F10 的差异在于 F13 的注入是 PRD 验收标准本体（F10 的注入非验收标准）；若不纳入，M7 手工验证降级为「数据源单测 + 0.2.0 联调」 |
| Q3 | F6 注入范围：MVP 注入**全部 open 伏笔**（priority 降序，F6 预算层裁剪，本 spec 设计）；是否需要**仅高优先级**（如 priority ≥ 阈值）或**按叙事位置过滤**（如只提醒当前叙事位置之前埋的伏笔）？ | 影响 ForeshadowingSource.collect 行为与注入质量；过滤逻辑若需要则 collect 需新增参数、F6 契约随之扩展 | 建议：MVP 全部 open + priority 降序（伏笔量级小，F6 dynamic 层裁剪已保证不超预算；阈值过滤是重复裁剪逻辑，YAGNI）；按章节/叙事位置过滤归 Phase 2+（叙事位置数据走 F12 事件，§5.3） |

---

*本文档为 F13 功能规格（What），实施步骤（How）见后续 `specs/f13-foreshadowing-service/plan.md`。所有里程碑验收以本节 M1-M8 为准。*
