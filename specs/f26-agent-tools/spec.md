...
**参考 ADR**: ADR-015（LangChain 隔离）、ADR-018（测试分层）、ADR-019（编号口径）、adr/agent/ADR-035.md（编排引擎=Deep Agents harness 0.7.5，v6 修订）
...
- **F27 spec 必须含重试护栏（硬性前置条件）**：最终 AIMessage content 为空 → 自动重试（附工具结果重申「请输出正文」）→ 仍空则 `terminated_by_guardrail`（映射 FAILED，与升级路径 v1.1 adr/agent/ADR-034.md 一致）。F27 起草时本条为强制验收项。
...
| 决策 | 方案 | 备选（否决理由） |
|------|------|-----------------|
| 编排框架 | **deepagents 0.7.5（v1.1 修订，2026-08-10 用户拍板，adr/agent/ADR-035.md 待修订）** | LangGraph 手写（不同 Agent 挂不同 skill 需自研 SKILLS_BY_ROLE 等效机制；deepagents 官方一等公民） |
...
**Spec 版本**: 1.0
**日期**: 2026-08-29
**依据**: adr/agent/ADR-043.md（工具面矩阵 §1 + 分阶段 §4）+ PRD §6.1 + 源码枚举（2026-08-29 核实）
...
**参考 ADR**: adr/agent/ADR-043.md（工具面矩阵 + 删除授权模型 + 分阶段）
**状态**: 待实现 🔲
