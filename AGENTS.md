...
| `adr/README.md` | ADR 索引（改代码前先查；25 条有效） |
...
| 0.3.1 | 质量加固补丁（milestone #9）：#86 LLM 客户端修复 ✅（PR #108：timeout→request_timeout + zhipu 注册 + audit 路由）· #87 LangGraph 状态重构 ✅（PR #110：StateGraph(dict)→TypedDict+reducer，节点增量返回，type: ignore 清零）· #92 真实 AI CI job 🔄 · #104 覆盖率补全 🔄 |
...
| 日志 | Loguru ≥ 0.7 | |
| 测试 | pytest ≥ 8.0 + pytest-asyncio + pytest-cov | 覆盖率 ≥ 70% |
...
| `adr/` | ADR 决策记录（索引 `adr/README.md`，25 条有效） |
