...
| 0.6.0 | F21 导出 · F22 全文搜索（✅ PR #216）· 世界观三连（**#173 地点树 ✅ PR #215** / **#174 地图视图 ✅ 本 PR** / #175 跨书复用）· CLI 恒经 HTTP（#169 ✅ PR #213）· E2E 设置页补全（#141）· 设定库分类实体创建（#196：角色/世界观等手动创建，当前占位跳写作页）· default_words 全局值重启加载（#198：设置页初始值读 fetchSettings）· 设置保存反馈统一化（#199：tray hint/close behavior 等切换顶部「已保存」） |
