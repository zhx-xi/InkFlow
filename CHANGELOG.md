...
- **写作链上下文注入真实生效（#1175/#1176/#1182/#1186/#1179/#1178/#1200/#1231/#1232/#1234/#1253）**：
  book 轨 / F3 确定性轨 / agentic 单章轨三轨统一接入 F6 上下文装配——此前 3 份硬编码 brief 副本、
  `NullProvider` 产线空转、伏笔未注入、`writing_style` 与章级 `writing_requirements` 零消费；
  大纲按 `level` 分块 + 只注入摘要 + 卷/章按当前章节匹配；新增 `SummarySource` 适配器把前文摘要
  接入组装通道。
- **F34 审计闭环（#1174/#1177/#1204/#1266/#1267/#1275/#1276/#1269/#1272/#1280/#1287）**：
  审计改读章节正文（原「拿大纲审大纲、score 凭空生成」）且 book 轨接入 F34 漂移检测；补
  「前后章连贯性」「大纲符合度」两类 check_type；**审计结果首次有消费方**——写作链按审计结论阻断
  （全自动必阻断，交互式交用户决定；补登 `blocked` 终态与 `needs_review` 章态）；修打包产物审计
  恒 `degraded`（未显式传 model）与无模型时 422（改降级，恢复 F34 §5.3 契约）。
- **卷级轨承接（#1187/#1286）**：同卷章节写前定钩子 + 写后 F34 卷级审计。
- **UUID 身份键（#1134 批 1-4 / ADR-060）**：统一 `new_uuid()`（UUIDv7）+ 机器标识；26 张 int PK
  表补 `uuid` 身份列 + uuid5 回填 + 唯一索引；新增 `require_uuid_pk` 契约入口；`repo.get` 入参
  收窄到 UUID 并退役 int 兼容面（22 份 `_to_int_id`）。为 2.0.0 云同步铺路。
- **内核单例与并发（#1237/#1244/#1192/#1195/#1317/#1335/#1380/#1385）**：GUI spawn 内核补
  `--port-file`（与 CLI 同源，恢复单例语义）；存活期互斥被占后的复用窗口对齐调用方 timeout；
  启动对账加实例归属判据（修新实例把别实例 running 中的 book run 打成 failed）；
  `%TEMP%\inkflow-kernel.log` 启动期归档轮转（修无上限膨胀，实测 918MB / 8826 次启动累积）。
- **角色关系数据面统一（#495/#1258/#1259）**：`character_relations` 并入 `knowledge_relations`。
- **GUI 收口**：时间线双序轴向重构（叙事序=章轴 / 世界序=纪元轴，#1374/#1395）+ 时间轴渲染与
  行内编辑删除入口（#1301/#1313、#1302/#1315）· 世界观条目页四处 + 分类校验/新建入口 + 树拖宽
  （#1375/#1394、#1321/#1345、#1322/#1346）· 知识图谱连线/节点集（#1325/#1357）+ 跨实体关系
  MCP 工具（#1359/#1365）· 写作页右栏 2:1 铺满与左右栏尺寸持久化（#1378/#1393、#1397/#1399）·
  上下文注入面板按大纲预选 + 一键清除 + 章级回显（#1379/#1389、#1342/#1343、#1344/#1348、
  #1349/#1354）· 伏笔行字段渲染（#1324/#1347）· 草稿驳回落位（#1377/#1383）· 提取分页公共组件
  覆盖长列表页（#1300/#1314）· 分页 total 随筛选重算（#1320/#1351）· 内置 agent 权限修正 + 6 个
  方法论 skill 内容充实（#1327/#1355）。
- **F23 变更推送批次 B（#1090/#1213）**：其余 14 域接入数据面变更事件（draft 除外）。
- **工程治理**：GUI 三件同步纪律 + 双件套门禁（AGENTS.md §4.6 + `ci_cd/check_gui_spec_sync.py`，
  #1326/#1337）· 页规格头部指针强制化（#1338/#1367）· 原型/页规格回填与截图工具链治理（#1366、
  #1361/#1362、#1363/#1368、#1400/#1401、#1402/#1403）· 迁移独立 job + ORM↔迁移漂移门禁
  （#1005/#1006/#1256）· ruff format 存量收敛 + format 硬门禁 + pre-commit/CI 版本统一
  （#1148/#1214、#1215/#1216）· `coverage-function` 改只合并报告（19.5min → 0.6min，#1206/#1207）·
  workflow `paths:` 触发面补齐（#1292/#1293/#1294）。
- **发布与验证**：rc1 → rc6 六轮预发布 + 完整用户旅程（stage0 → stage8）验证。

### 变更

- **上下文注入 override 空列表语义升级（#1235/#1252）**：空列表 = 不注入，缺省 = 全注入；
  面板勾选可添加/可删除/可删空。
- **`chapter_summary` 保留为未实现枚举项（#1236/#1251）**：补未实现标记，防继续被当活契约消费。

### 修复

- **写作链**：F3 轨 `project_repo.get(UUID)` 抛 TypeError 全崩（#1230/#1242）· chat/agentic 轨模型
  解析回退项目 `config.model`（#1298/#1308、#1309/#1311）· 正文首行章节标题回声剥离（#1261/#1274、
  #1318/#1340）· book 轨空正文致整章 failed（#1316/#1336）、兜底路径产 content-block repr 且占真
  章节槽位致正文静默丢失（#1262/#1273）· book run 安全闸判据与数据状态解耦 + 「不删正文重跑」
  重置出口（#1265/#1281、#1282/#1284）· 章级 `writing_requirements` 真实数据源达 book brief
  （#1200/#1205）。
- **提取与数据**：setting 提取新建条目未挂 `parent_id` 撞根单例索引恒 500（#1297/#1306、#1372/#1384）·
  项目 `hard_delete` 不级联清理无 FK 子表族致孤儿行残留（#1371/#1386）· 角色 `role_rank` 提取链
  补齐 + MCP 工具签名去默认值（#1299/#1303/#1312）。
- **GUI**：编辑区 CSS `text-indent` 移除，首行缩进由内容层唯一承担（#1238/#1250）· ChatPanel 泄漏
  `setTimeout` 致 unmount 后 setState（#1248/#1249）· readiness 查询补轮询/重试 + 写成功补发推送
  重查（#1218/#1224、#1226/#1228）。
- **内核与环境**：打包版 CLI 在中文 Windows(936) 下 stdout 为 GBK → 强制 UTF-8 stdio 且环境无关
  （#1240/#1243）· GUI 与 CLI 各起一个内核永不共用（#1237/#1244）· 存活期互斥并发冷启动误报
  「既有实例冲突」（#1192/#1195）。
- **RAG**：`/vector/retrieve` 间歇 500（210 次/16 天）补异常栈落盘 + `RAGUnavailableError` 改 503
  （#1381/#1390）。
- **e2e / CI 稳健性**：托盘族、library 世界观树/筛选族、library pin-dialog、settings `gotoNav`
  就绪门、tray 关闭行为、rag-stale-banner 跨重启、8 个 no-gate spec 渲染层就绪门等 flaky 整治
  （#1194/#1198/#1211、#1212/#1220、#1219/#1225、#1227/#1229、#1239/#1255、#1257/#1279、
  #1283/#1285/#1289、#1295/#1305、#1307/#1310、#1208/#1222）。
- **文档与治理**：规范体系 SSOT 归位（AGENTS.md 427→150 行，宪法/spec 漂移修复，#1190 批 A-C）·
  spec 状态标记谎报收口与失效测试路径引用修复（#1184b/#1209、#1210/#1217、#1197/#1199、
  #1260/#1278）· 历史证据归档入 `docs/evidence/` 并全仓禁止引用 `.hermes/`/`.tmp/`（#1332）。

### 修复（0.14.0 已知遗留收口）

- **#1171**：CLI `ensure_kernel` 不复用 `--port-file` → 每条命令泄漏一个内核 + `kernel.json` 互覆
  （rc9 打包版实测 35 实例）→ 互斥统一 `kind`+`data_dir`、dev 同 `data_dir` 单内核、并发竞态等待
  （ADR-059 修订；PR #1189）。
- **#1172**：多轮历史重建丢弃 `reasoning_content` → 思考模型收到空白思考链，多轮质量静默退化
  （v0.14.0 实测 36 次告警）→ `ChatMessage` 加字段 + 全链透传（PR #1189）。
