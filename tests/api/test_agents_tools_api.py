"""#954 F58 §3.2 契约升级 — GET /api/v1/agents/tools 工具目录
（26 自定义工具 + domain/op + 路由顺序硬契约）。

自 tests/api/test_agents_api.py 拆出（#838 统一 35 工具目录契约，2026-08-31）。
#954 F58 §3.2 契约升级（F58 spec 取代 #838 平铺目录契约）：
- 目录从全量 35 工具收敛为 26 自定义工具（is_core=True 的 9 核心工具不进目录：
  agent_run/agent_call + 7 个 delete/memory_remove）。
- 每项 6 键 → 8 键：新增 `domain` / `op` 两键（GRANT_TOOL_MAP 格值，
  contract-954 §2.1 GRANT_TOOL_MAP + §4 catalog 升级）。
- CORE_TOOL_NAMES 断言反转：核心 9 工具「不出现在 items」。
参考：contract-954 §8（契约升级迁移处置）+ specs/f58-agent-tool-scope/spec.md
§3.2（工具目录端点）+ spec §2.1（GRANT_TOOL_MAP）。

【G】守护：`test_tools_route_not_swallowed_by_agent_id`（200 路由顺序，既有行为
守护，契约升级不破坏，保持不动）；catalog 用例随 #954 契约升级改断言（RED）。
"""
...
"""ALL_TOOL_SPECS 全量 35 工具全集（9 组，含 9 核心工具，仅作全集参考；
catalog 端点按 #954 F58 §3.2 只返回 26 自定义工具）。"""
...
#954 F58 §3.2：这 9 个工具**不在** catalog 响应 items 中。"""

EXPECTED_TOOL_NAMES = set(ALL_TOOL_NAMES)
"""ALL_TOOL_SPECS 全量 35 名字全集（含核心）。"""

CUSTOM_TOOL_NAMES = EXPECTED_TOOL_NAMES - CORE_TOOL_NAMES
"""#954 F58 §3.2 自定义 Agent 可见工具（allow_custom_agent=True，is_core=False，
= catalog 响应 items 全集，26 个，= TOOL_REGISTRY）。"""
...
"""#954 F58 §3.2 每工具 (domain, op) 格值表——26 自定义工具逐字来自
contract-954 §2.1 GRANT_TOOL_MAP（值 = TOOL_NAME_TO_CELL 反查）。"""
...
def _assert_tool_catalog(items: list) -> None:
    """工具目录整体契约（#954 F58 §3.2）：26 自定义工具全集 + 每项 8 键 +
    核心 9 工具不出现在 items + 每工具 domain/op == GRANT_TOOL_MAP 格值。"""
    assert isinstance(items, list)
    names = [t["name"] for t in items]
    assert len(items) == 26, f"工具目录应含 26 自定义工具: {len(items)}"
    assert set(names) == CUSTOM_TOOL_NAMES, f"工具目录名字全集不符: {names}"
    by_name = {t["name"]: t for t in items}
    for tool in items:
        _assert_tool_entry(tool)
    # #954 F58 §3.2：核心 9 工具不进目录（is_core 过滤），断言反转为「不出现在 items」
    for n in CORE_TOOL_NAMES:
        assert n not in names, f"核心工具 {n} 不应出现在目录（#954 F58 is_core 过滤）"
    # #954 F58 §3.2：26 自定义工具 allow_custom_agent=True / is_core=False，
    # domain/op == contract §2.1 GRANT_TOOL_MAP 格值
...
class TestToolCatalog:
    ...
    async def test_tools_catalog_26_custom_tools_with_domain_op(
        self, client, db_session, override_get_db
    ):
        """【R】#954 F58 §3.2：26 自定义工具全集 + 每项 8 键（+domain/op）+
        核心 9 工具不在 items + 每工具 domain/op == GRANT_TOOL_MAP 格值。"""
        resp = await client.get(ENDPOINT_TOOLS)
