...
from inkflow.api.app import (
    app,  # 必须先于 stub 行导入（GREEN 时 app 导入链注册真模块/真 deps）
)
...
class TestChatAgentStreamSuccess:
    ...
    async def test_agent_stream_calls_service_with_prompt_and_context(
        self,
        override_chat_agent_service,
        mock_chat_agent_service,
        override_agent_run_repo,
    ):
        """mock stream_events() 收到 prompt + chapter_context（keyword 透传）。"""

        payload = _payload("帮我写一段打斗场景")
