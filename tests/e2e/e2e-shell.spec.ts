...
test('启动闭环：窗口出现（title 含 InkFlow）+ 内核进程存在 + /health 200 + M5 安全基线', async () => {
  ...
  try {
    ...
    // #106 自绘窗口按钮后 INKFLOW_API 含 windowControls 第 3 键；#167 F31 含 settings 第 4 键；#654 项目导出含 file 第 5 键（preload 契约升级）
    expect(Object.keys(api!).sort()).toEqual(['baseURL', 'file', 'settings', 'token', 'windowControls']);
