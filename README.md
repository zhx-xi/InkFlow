...
- **思考档位（reasoning effort）**：chat 对话每轮可选思考级别（含「跟随模型默认」）；项目写作链与全自动写作按设定页「Agent思考强度设定」执行；模型思考能力自动探测 + 手动覆盖
- **GUI 桌面端**
  - 项目管理
  - 写作页（三栏布局，SSE 流式输出）
  - Agent 配置
...
| 0.12.0 | AI 全自动写作 + 内置内容补全（Agent 自主编排 · Chat 接入 deepagents · F49 长期记忆衰减 · 卷数据模型统一 F56 · 项目标签化 · 设定库 GUI 补全批 · MCP 分发引导） | ✅（v0.12.0 正式发布，65/65 issues 全关） |
| 0.12.1 | 0.12.0 rc10 GUI 反馈 + 后续修复批 | ✅（v0.12.1 正式发布，77/77 issues 全关） |
| 0.13.0 | Chat Agent 工具矩阵 + Scope 授权（F58）+ 写章链路根治（F44：唯一草稿守卫/树常显审批/锚点传递/同章幂等）+ planner 质量 + 地图/写作页修复 | ✅（v0.13.0-rc6 验证通过，61/61 issues 全关；待正式发布） |
