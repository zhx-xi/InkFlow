...
| [ADR-019](ADR-019.md) | 版本里程碑管理 — SemVer + 1.0.0 = 本地完全可用（v7：里程碑重排 v2 + skills 后移 v3 + F20 MCP 后移 v4 + F25 daemon 移除 v5 + 0.7.0 Agent 化 v6 + 0.8.0 编排完全体 v7） | ✅ 已接受（v7 修订） | 2026-08-01（v7 2026-08-13） |
...
| [ADR-039](ADR-039.md) | Skills 文件系统真源 — 去表（DB 退役）+ 启动回补 + 一次性迁移 + Agent.skill_ids 目录名引用（#522，D3 拍板 2026-08-20） | ✅ 已接受 | 2026-08-20 |
| [ADR-040](ADR-040.md) | 对话输入机密脱敏（A+B 双兜底） | ✅ 已接受 | 2026-08-23 |
| [ADR-041](ADR-041.md) | chat 会话可重放 — 复用 agent_runs 落 run 决策 | ✅ 已接受 | 2026-08-24 |
| [ADR-042](ADR-042.md) | LangSmith 可观测性追踪接入 — 默认关 + SaaS 优先（F50，#629） | ✅ 已接受 | 2026-08-24 |
| [ADR-043](ADR-043.md) | 打包产物 Debug 模式 — 统一开关 + 日志落点 + GUI DevTools + serve 可直达（F51，#713/#714/#715） | ✅ 已接受 | 2026-08-27 |
...
- **版本里程碑**: SemVer 版本号管理；1.0.0 = 本地完全可用（CLI+GUI+skills+MCP）；2.0.0 = 云端（云 Web + 用户 API + Admin 后台 + GUI 远程模式）（ADR-019 v7，2026-08-13：v2 里程碑重排 + v3 skills 后移 1.0.0 + v4 F20 MCP 后移 1.0.0 + v5 F25 daemon 移除 + v6 0.7.0 Agent 化主线 + v7 0.8.0 编排完全体）
